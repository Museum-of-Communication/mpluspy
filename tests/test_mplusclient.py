...
def test_request_with_placeholders(
    mock_mplus_client, mock_request, mock_response, mock_xml_file
):
    mock_request.return_value = mock_response
    url_placeholders = {"id": 123}
    xml_placeholders = {"limit": 10, "offset": 0}
    response = mock_mplus_client.request(
        "example-request-2",
        xml_placeholders=xml_placeholders,
...
def test_request_url(mock_mplus_client):
    ...
    url = mock_mplus_client._MPlusClient__request_url(
        "example-request-1", placeholders={"key": "value"}
    )
    assert url == "https://example.zetcom.com/MpWeb/ria-ws/application/example"
...
def test_request_url_with_placeholders(mock_mplus_client):
    url = mock_mplus_client._MPlusClient__request_url(
        "example-request-2", placeholders={"id": "123"}
    )
...
def test_request_url_missing_placeholders(mock_mplus_client):
    with pytest.raises(KeyError):
        mock_mplus_client._MPlusClient__request_url(
            "example-request-2", placeholders={"key": "value"}
        )
